...
void RenderObject::setTransform(float dx, float dy, float scale) {
  qDebug() << _simpleShader.get();
  _simpleShader->setTransform(dx, dy, scale);
}

void RenderObject::setColour(float r, float g, float b, float alpha) {
  //  _simpleShader->setColour(r, g, b, alpha);
}
...
void RenderObject::initializeBase(QOpenGLFunctions *f) {
  ...
  _isInitialized = true;
  _simpleShader.reset(new SimpleShader);
  _simpleShader->setTransform(0, 0, 1);
}
